  ...
  describe('given a position segmentation data', function () {
    it('should return a Promise with a Domain Position and AppNexus data loaded and Position displayed in DOM', function (done) {
      ...
      openAds.addPosition({
        ...
        .then(position => {
          ...
          expect(position.status, `The position status is equal to ${position.status}, instead it should be equal to 'POSITION_VISIBLE'`).to.be.equals(POSITION_VISIBLE)
          done()
        })
    ...
    it('should return a rejected promise due a nonexistent position', function (done) {
      ...
      openAds.displayPosition({id: 'id not found'})
        ...
        .catch(error => {
          expect(error.name, `DisplayPosition return a rejected Promise, in this case, we expect an error with name: 'PositionNotFoundException', but we have received a ${error.name}`).to.be.equals('PositionNotFoundException')
          done()
        })
    })
