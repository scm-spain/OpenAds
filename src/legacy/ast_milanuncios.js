    ...
    ApntagManager.prototype.pushCommands = function() {
        ...
        function defineTags(tag) {
          if (this.utagData.middle1_div_id !== tag.targetId) {
              ...
              if (this.utagData.x65_div_id == tag.targetId)
              {
                if (X65PLSCREEN >= this.utagData.pl_screen_view && "list" === this.utagData.event_name) {
                  var numAds = document.getElementsByClassName('aditem').length, posAd;
                  if (numAds === 0 ) {
                    posAd = document.getElementsByClassName('nohayanuncios')[0];
                  } else if (numAds < 5 && numAds > 0) {
                    posAd = document.getElementsByClassName('aditem')[numAds - 1];
                  } else {
                    posAd = document.getElementsByClassName('aditem')[4];
                  }
                ...
                if (posAd) {
                  ...
                  divAd.style.margin = '10px';
                  posAd.parentNode.insertBefore(divAd, posAd.nextSibling);
                  if(document.getElementById("pubMA"))
                  {
                    top.document.getElementById("pubMA").innerHTML = '<div style="width: 75px; line-height: 20px; text-align: center; padding: 2px; font-size: 12px; font-family: verdana, arial, sans-serif; color: rgb(0, 0, 0); background-color: rgb(238, 238, 238);">Publicidad</div>';
                  }
                }
              }
            }
...
(function(globalApnTag, globalUtagData) {
    ...
    if (healthCheck) {
        ...
        //Desktop
        if(!globalUtagData.is_msite){
          if(globalUtagData.event_name === "list") {
          ...
          else if(globalUtagData.event_name === "detail") {
            WINDOW_SIZES_TAGS_CONFIG = [
                {
                    screenConfig: '4',
                    tags: [{
        ...
        //Mobile
        else if(globalUtagData.is_msite){
        if(globalUtagData.event_name === "list") {
          WINDOW_SIZES_TAGS_CONFIG = [
              {
                  screenConfig: '4',
